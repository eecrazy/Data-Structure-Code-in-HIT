...
void Swapper(void **vpnt1, void **vpnt2)/*定义指针，并将待交换的地址存入。这么用居然在windows下 
可以，Linux下报错。*/
{
    void *tempPnt = *vpnt1;
    *vpnt1 = *vpnt2;
...
{
    UINT minLen = ((DataCode*)op1)->len < ((DataCode*)op2)->len ? ((DataCode*)op1)->len :  
((DataCode*)op2)->len;
    UINT i;
...
void RebuildHuffmanTreeOrder(HuffmanTreeNode **pTreeHead, UINT start, UINT end,UINT depth,  
...
void GetHuffmanCode(const HuffmanTreeNode *pTreeHead, UCHAR code[CODE_SIZE],USHORT codeLen,  
...
void WriteHead(FILE *fp, const DataCode dataCodeArr[CODE_SIZE])
{
    ...
    for(i = 0; i < CODE_SIZE; i++)
    {
        ...
        {
            memcpy(codeStr, dataCodeArr[i].pCode + j, UCHAR_SIZE * (dataCodeArr[i].len %  
CHAR_BIT));
            codeBuffer = ConvertToCodeData(codeStr);
            fwrite(&codeBuffer, UCHAR_SIZE, 1, fp);
        }
...
int UnzipFile(const char *sourceFileName, const char *targetFileName)
{
    ...
    while(!feof(sourceFp) || codeStreamLen > 0)
    {
        ...
        if(pTreeNode->rightChild != NULL) return 1;/* 对当前节点进行判断，若其左右孩子不为空， 
则为非节点，有错 */
