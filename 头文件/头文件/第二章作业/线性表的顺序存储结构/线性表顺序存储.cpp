...
//返回最后一个元素Type back();
复杂度为O(1);
//搜索元素bool search(const Type&searachItem);
复杂度为O(n);
//在表头插入元素void insertFirst(const Type &newItem);
复杂度为O(1);
//在表尾插入元素void insertLast(const Type &newItem);
复杂度为O(1);
//删除节点void deleteNode(const Type&deleteItem);
复杂度为O(1);
*****************************/
